...
%%%%%%%%%%%%%%%% LOAD VIDEO
videoName = "../bee_tracking/data/S2/S2_cropped";
videoFileName = "./data/" + videoName + ".MP4";
% croping the video
limits.rowStart    = 301;    limits.rowEnd      = 700;    limits.colStart    = 201;    limits.colEnd      = 700;
...
% writerObj = VideoWriter('./output_videos/simulation_S2_bee_waggles_only_framediff.avi');
...
for iSegment = 1:2
    ...
    dGreyScaleArray    = zeros(imageHeight/2,imageWidth/2,nFrame,'single');
    ...
    for iFrameOriginal = startFrame:  endFrame
        ...
        if iFrame>1
            ...
            % normalising the frame, greyscale (remove vecnorm)
...
%         writeVideo(writerObj, F);
        ...
        if ~isempty(r) && (c-rad25>0) && (c+rad25<size(dGreyScaleArray,2)) && (r-rad25>0) && (r+rad25<size(dGreyScaleArray,1))
            ...
            td.frameID(iWaggleEvent)= iFrame;
        end
