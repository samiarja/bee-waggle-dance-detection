...
videoFileName = './data/20210803t1301d050m.MP4';
...
if RECORD_VIDEO
    set(gcf,'color','k')
    globalFrameRate                 = 60;
    vid1           = VideoWriter(['beeWaggle4Panels_v2'],'MPEG-4');
...
%     figure(423423); histogram(dRgbFrameArray(1e4:1e5)); yLog;
    %%%%  3D convolution
    sigma        = 10;
    Tau          = 36;
    delArray     = 1:nDel;
    ...
    if SHOW
        figure(42342);plot(waggleFilt1,'*-'); grid on;
    end
    ...
    if SHOW
        ...
        figure(5345); clf; hold on;
        plot(1:nFrame,maxWaggleVal); box on; grid on;
        plot(1:nFrame,meanWaggleVal); box on; grid on;
        plot(1:nFrame,maxWaggleVal./meanWaggleVal); box on; grid on;
    end
    ...
    for iFrame = 1:nFrame
        ...
        if ~isempty(r)
            ...
            kk = 10;
            angleArrayActivation= movmedian(angleArrayActivation,kk);
        ...
        if SHOW
            if iFrame ==1
                ...
                set(imh2,'CDATA',   uint8((dRgbFrameArray(:,:,:,iFrame))+127))
