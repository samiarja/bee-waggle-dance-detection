...
if INTERPOLATE
    ...
    for k=1:numel(labels)-1
        ...
        if numel(datacell_before{1}(:,1)) == 1 && numel(datacell_after{1}(:,1)) == 1
        ...
        elseif numel(datacell_before{1}(:,1)) == 2 && numel(datacell_after{1}(:,1)) == 2
            x_coor_before = datacell_before{1}(2,1);
            y_coor_before = datacell_before{1}(2,2);
            angle_before  = datacell_before{1}(2,4);
            
            x_coor_after = datacell_after{1}(2,1);
            y_coor_after = datacell_after{1}(2,2);
            angle_after  = datacell_after{1}(2,4);
        ...
        elseif numel(datacell_before{1}(:,1)) == 3 && numel(datacell_after{1}(:,1)) == 3
            x_coor_before = datacell_before{1}(3,1);
            y_coor_before = datacell_before{1}(3,2);
            angle_before  = datacell_before{1}(3,4);
            
            x_coor_after = datacell_after{1}(3,1);
            y_coor_after = datacell_after{1}(3,2);
            angle_after  = datacell_after{1}(3,4);
...
%     td_gt.frameID = (firstFrameLabel:firstFrameLabel+frameIDNAN(1)-2)';
    ...
    for k=1:numel(labels)-1
        ...
        datacell = textscan(labelID, '%f%f%f%f', 'Delimiter',',', 'CollectOutput', 1);
        x_coor = datacell{1}(1);
        y_coor = datacell{1}(3);
        angle  = datacell{1}(4);
...
%% setup segmentation output into cells
...
% td = [];
% for id = 1:399
%     load("/media/sam/Samsung_T5/MPhil/Code/bee_tracking/data/integration/bee_id_" + id + "_trajectory.mat")
%     td.beeTrajectory{id} = beeTrajectory';
%     td.beeTrajectory = td.beeTrajectory';
% end

figure(6776);
for idx = 1:numel(td.beeTrajectory)
    trajectory = td.beeTrajectory{idx,1};
    time = 1:numel(trajectory(:,1));
    plot3(trajectory(:,1), trajectory(:,2),time);hold on;grid on
end
plot3(td_gt.x,td_gt.y,td_gt.frameID,'LineWidth',2);
xlim([min(td_gt.x) + 5 max(td_gt.x) + 5])
ylim([min(td_gt.y) + 5 max(td_gt.y) + 5])
